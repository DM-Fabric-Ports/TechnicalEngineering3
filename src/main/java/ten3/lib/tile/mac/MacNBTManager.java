...
public class MacNBTManager {
	...
	public void rdt(CompoundTag nbt) {
		...
		for (int i = 0; i < t.tanks.size(); i++) {
			t.tanks.get(i).readFromNBT(nbt.getCompound("fluid" + i));
		}
	...
	public void wdt(CompoundTag nbt) {
		...
		for (int i = 0; i < t.inventory.getContainerSize(); i++) {
			nbt.put("item" + i, t.inventory.getItem(i).copy().save(new CompoundTag()));
		}
		for (int i = 0; i < t.tanks.size(); i++) {
			var nbtt = new CompoundTag();
			t.tanks.get(i).writeNbt(nbtt);
			nbt.put("fluid" + i, nbtt);
		}
